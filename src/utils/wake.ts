...
class Wake
{
        ...
        if (nativeWakeLock()) {
            const handleVisibilityChange = () => {
                if (this._wakeLock !== null && document.visibilityState === "visible") {
                    this.enable().catch(e => log.error(e))
                }
            }
    ...
    private addSourceToVideo(element: HTMLVideoElement, type: string, dataURI: string) {
    ...
    disable() {
        if (nativeWakeLock()) {
            if (this._wakeLock) {
                this._wakeLock.release().catch(e => log.error(e))
            }
            this._wakeLock = null
        } else if (oldIOS()) {
            if (this.noSleepTimer !== null) {
                log.warn(`Wake now disabled for older iOS devices.`)
